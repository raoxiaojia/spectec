...
let itp_of_mixop (m: mixop) : term = 
  match m with
  | [{it = Atom id; _}]::tail when List.for_all ((=) []) tail -> E_ident id
  | mixop ->
    E_unsupported (let s =
      String.concat "%" (List.map (
        fun atoms -> String.concat "" (List.map Il.Atom.string_of_atom atoms)) mixop
      )
    in
    "`" ^ s ^ "`")
...
and itp_of_typ t =
  match t.it with
  | VarT (id, as1) -> T_app ((T_ident id.it), (List.map itp_of_arg_as_type as1))
...
and itp_of_exp e : term =
  ...
  | SliceE (e1, e2, e3) -> E_app (itp_of_exp e1, [E_unsupported ("[" ^ Il.Print.string_of_exp e2 ^ " : " ^ Il.Print.string_of_exp e3 ^ "]")])
  | UpdE (e1, p, e2) -> (* Need to differentiate between list and record update *)
    E_unsupported (
      Il.Print.string_of_exp e1 ^
        "[" ^ Il.Print.string_of_path p ^ " = " ^ Il.Print.string_of_exp e2 ^ "]")
  | ExtE (e1, p, e2) ->
    E_unsupported (
      Il.Print.string_of_exp e1 ^
        "[" ^ Il.Print.string_of_path p ^ " =.. " ^ Il.Print.string_of_exp e2 ^ "]")
  | StrE efs -> E_unsupported ("{" ^ concat ", " (List.map string_of_expfield efs) ^ "}")
  | DotE (e1, atom) ->
    E_unsupported (Il.Print.string_of_exp e1 ^ "." ^ Il.Print.string_of_mixop [[atom]] ^ "_" ^ Il.Print.string_of_typ e1.note)
  ...
  | IterE (e1, iter) -> E_unsupported (Il.Print.string_of_exp e1 ^ string_of_iterexp iter)
  ...
  | TheE e1 -> E_unsupported ("!(" ^ Il.Print.string_of_exp e1 ^ ")")
  ...
  | SubE (e1, t1, t2) ->
    E_unsupported ("(" ^ Il.Print.string_of_exp e1 ^ " : " ^ Il.Print.string_of_typ t1 ^ " <: " ^ Il.Print.string_of_typ t2 ^ ")")
...
let itp_of_param p : binder =
  match p.it with
  | ExpP (id, t) -> ((if id.it = "_" then "default_name" else id.it), itp_of_typ t)
...
let rec itp_of_def d =
  ...
  | DecD (id, ps, t, clauses) ->
    FuncD (id.it, List.map itp_of_param ps, itp_of_typ t, E_match ("placeholder", List.map (itp_of_clause id) clauses))
  | RecD ds ->
    MutualD (List.map itp_of_def ds)
  | HintD _hints ->
